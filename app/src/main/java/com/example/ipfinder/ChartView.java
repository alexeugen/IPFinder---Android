...
//    }
    ...
    public ChartView(Context context, @Nullable AttributeSet attrs) {
        super(context);
        db = IpFinderDB.getInstance(context);
        ArrayList<Category> categories;
        ArrayList<IpInfo> nbrOfIps;
        categories = (ArrayList<Category>) db.getCategoriesDao().getAll();

        Map<String, Integer> categorySource = new HashMap<>();
        for(Category elem : categories) {
            nbrOfIps = (ArrayList<IpInfo>) db.getIpInfoDao().getByCategory(elem.getCategoryId());
            categorySource.put(elem.getName(), nbrOfIps.size());
        }

        this.source = categorySource;
        this.paint = new Paint(Paint.ANTI_ALIAS_FLAG);
        paint.setColor(Color.BLACK);
        labels = new ArrayList<>(source.keySet());
        random = new Random();
